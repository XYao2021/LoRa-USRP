...
class SlotStrobe(gr.basic_block):
    ...
    """
    def __init__(self, agent_id=0, frame_period_ms=1000):
        ...
        self._seq           = 0
        self._n_sent        = 0
        self._start_delay_s = 0.0       # seconds to wait before first packet
    ...
    def arm(self, n_packets: int, message: str = None,
        ...
        """
        Arm for one burst.
        start_delay_s: seconds to wait after arm() before sending first packet.
                       Server sets this to i * (n_packets * period_ms / 1000)
                       so agent i does not overlap with agent i-1.
        """
        with self._lock:
            self._n             = n_packets
            self._n_sent        = 0
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            # Wait for our time slot within the shared slot window
            if start_delay_s > 0.0:
                print(f"[TX{self._agent_id}] Waiting {start_delay_s:.1f}s "
                      f"before burst (TDM offset)")
                self._stop.wait(start_delay_s)
                if self._stop.is_set(): break
            ...
            while sent < n and not self._stop.is_set():
                ...
                # Payload format: "AGT:{id} SEQ:{seq:05d} {message}"
                # AGT prefix lets RX filter by agent_id at packet level.
                # This works identically on USRP (gr-lora_sdr) and SX1276
                # (Arduino), making FDD unnecessary.
                payload = f"AGT:{agent_id} SEQ:{seq:05d} {message}"
                print(f"[TX{agent_id}] ({sent+1}/{n}) {payload!r}")
...
class ServerCommandHandler:
    ...
    def _handle(self, cmd, conn):
        ...
        if c == 'set_gain':
            ...
            # start_delay_ms: server staggers agents so bursts don't overlap.
            # Agent i starts at i * (n_packets * period_ms) ms after slot open.
            start_delay_ms = int(cmd.get('start_delay_ms',   0))
            start_delay_s  = start_delay_ms / 1000.0

            self.tb.set_tx_gain(gain)
            print(f"[TX{self.agent_id}] gain={gain} dB  "
                  f"n_packets={n_packets}  period={period_ms} ms  "
                  f"start_delay={start_delay_ms} ms")

            # Send ready ACK immediately (before the delay)
            self._send({
                "status"   : "ready",
            ...
            # Arm strobe — it will wait start_delay_s before first packet
            # Timeout = delay + burst duration + margin
            timeout = start_delay_s + n_packets * (period_ms / 1000.0) + 30.0
...
class lora_TX(gr.top_block):
    ...
    def __init__(self, center_freq=915000000, gain=30, samp_rate=250000,
        ...
        # ── Slot strobe ───────────────────────────────────────────────────────
        self.strobe = SlotStrobe(agent_id=agent_id,
